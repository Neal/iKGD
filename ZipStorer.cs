...
namespace System.IO.Compression
{
    ...
    public class ZipStorer : IDisposable
    {
		...
		/// <param name="_bytesToExtract">Number of bytes to extract; 0 for whole file</param>
        /// <returns>True if success, false if not.</returns>
		/// <remarks>Unique compression methods are Store and Deflate</remarks>
		public bool ExtractFile(ZipFileEntry _zfe, string _filename, ulong _bytesToExtract)
		{
			...
			bool result = ExtractFile(_zfe, output, _bytesToExtract);
			if (result)
				output.Close();
        ...
        /// <returns>True if success, false if not.</returns>
		/// <remarks>Unique compression methods are Store and Deflate</remarks>
		public bool ExtractFile(ZipFileEntry _zfe, Stream _stream, ulong _bytesToExtract)
		{
			if (!_stream.CanWrite)
			...
			uint bytesPending = (_bytesToExtract == 0) ? _zfe.FileSize : (uint)_bytesToExtract;
